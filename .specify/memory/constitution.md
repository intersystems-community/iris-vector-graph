...
1. **Auto-Increment Columns**:
   - ✅ IRIS syntax: `BIGINT NOT NULL IDENTITY` with separate `PRIMARY KEY (col)` constraint
   - ✅ CRITICAL: Must use `CREATE TABLE` (NOT `CREATE TABLE IF NOT EXISTS`) with IDENTITY
   - ❌ SQL standard: `BIGINT GENERATED BY DEFAULT AS IDENTITY`
   - ❌ Incomplete IRIS: `BIGINT IDENTITY PRIMARY KEY` (causes ERROR #5373)
   - ❌ With IF NOT EXISTS: `CREATE TABLE IF NOT EXISTS ... IDENTITY` (causes ERROR #5373)
   - ❌ PostgreSQL: `SERIAL PRIMARY KEY`
   - ❌ MySQL: `BIGINT AUTO_INCREMENT`
   - **Example**: `CREATE TABLE rdf_edges( edge_id BIGINT NOT NULL IDENTITY, ... PRIMARY KEY (edge_id) )`
   - **Rationale**: IRIS doesn't support IF NOT EXISTS with IDENTITY columns; use CREATE TABLE and handle "already exists" errors
