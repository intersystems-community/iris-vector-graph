...
1. **Auto-Increment Columns**:
   ...
   - ❌ **IRIS IDENTITY NOT SUPPORTED**: After 5 failed attempts, IDENTITY keyword causes ERROR #5373 in all variations
   ...
   - **Rationale**: IRIS IDENTITY keyword not universally supported - ERROR #5373 persisted across all syntax variations tested (with/without IF NOT EXISTS, with/without NOT NULL, inline/separate PRIMARY KEY). Manual ID management is the only reliable approach.
