...
- Python 3.11+ (per `pyproject.toml`); ObjectScript on IRIS 2025.1+ / 2026.1 community + `intersystems-irispython>=3.2.0` (Native API), `iris-devtester>=1.18.1` (test container lifecycle), existing IVG packages (164-seed-local-khop-fast-path)
- `^NKG` global (integer-indexed adjacency, populated by `Graph.KG.Traversal.BuildNKG()`); `^KG("out", 0, *, *, *)` fallback (string-keyed) (164-seed-local-khop-fast-path)
...
## Recent Changes
- 164-seed-local-khop-fast-path (v1.99.1): `engine.khop_seedlocal()` + routed `engine.khop()` for hops≤IVG_KHOP_SEEDLOCAL_MAX_HOPS (default 2). Walks `^NKG(-1, sIdx, *, *)` directly via `$Order`, bypassing Rust `KHopNeighbors` whole-graph serialization. 1-hop: 179µs → ≤150µs (1.95× faster) on 50K-node fixture. `^KG` fallback when `^NKG` not built; RuntimeWarning once-per-Python-process. `IVG_TEST_PORT` env var for `scripts/test-container.sh`. `Graph.KG.GraphIndex.EmptyHLL` compile fix (pre-existing `..EmptyHLL()` → `##class(...)` bug resolved). 12 e2e PASS + 1 xfail Bug S Cypher path + 5 unit tests.
