...
## Recent Changes
- 165-build-nkg-complete (v1.99.2): `engine.rebuild_nkg()` updated to call `BuildNKGComplete` (1 round-trip vs 7). New `Graph.KG.Traversal.BuildNKGComplete()` runs BuildNKG + Build2HopStats server-side; returns `"OS:<ec>:<nc>"` / `"RUST:<ec>:<nc>"` discriminated prefix. `IsLoaded()` guards fix non-trappable `<DYNAMIC LIBRARY LOAD>` in `InvalidateAdjCache` + `Build2HopExact` when libarno absent. EmptyHLL fix applied to main. 7 tests pass (4 unit + 3 e2e + 1 skipped).
