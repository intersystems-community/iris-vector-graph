...
- **CRITICAL: rdf_edges IDENTITY Syntax**: IDENTITY keyword NOT universally supported in IRIS (FIVE attempts needed)
  - Bug Evolution:
    - Attempt 1: `BIGINT GENERATED BY DEFAULT AS IDENTITY` → Invalid SQL statement
    - Attempt 2: `BIGINT IDENTITY PRIMARY KEY` → ERROR #5373
    - Attempt 3: `BIGINT NOT NULL IDENTITY` + `PRIMARY KEY (col)` + `IF NOT EXISTS` → ERROR #5373
    - Attempt 4: `BIGINT NOT NULL IDENTITY` + `PRIMARY KEY (col)` WITHOUT `IF NOT EXISTS` → ERROR #5373
    - Attempt 5 (FINAL): **Removed IDENTITY entirely** → Use manual ID management
  - Impact: **rdf_edges table creation failed**, causing kg_NEIGHBORHOOD_EXPANSION() and kg_PERSONALIZED_PAGERANK() to fail
  - IRIS Errors:
    - "Invalid SQL statement - ) expected, IDENTIFIER (GEN" (attempt 1)
    - "ERROR #5373: Class 'User..." (attempts 2-4, all IDENTITY variations)
  - **Key Discovery**: ERROR #5373 persisted across ALL IDENTITY syntax variations (with/without IF NOT EXISTS, with/without NOT NULL, inline/separate PRIMARY KEY)
  - **Root Cause**: IRIS IDENTITY keyword fundamentally incompatible in this context
  - **Solution**: Use manual ID management (application-generated IDs)
  - Final syntax: `CREATE TABLE IF NOT EXISTS rdf_edges( edge_id BIGINT PRIMARY KEY, ... )`
  - Location: `iris_vector_graph/schema.py:125-131`, `sql/schema.sql:22-28`
  - Reference: `sql/migrations/000_base_schema_iris.sql` (uses manual IDs, not IDENTITY)
  - Discovered by: HippoRAG2 team testing - **ALL v1.1.6 IDENTITY attempts failed!**
...
**v1.1.6 Bug (IDENTITY Syntax)**:
1. No contract test validating IRIS-native auto-increment syntax
2. `sql/schema.sql` also had the bug, so file-based tests wouldn't catch it
3. No integration tests actually creating rdf_edges and verifying table exists
4. **Root cause**: No live IRIS schema creation test in CI/CD pipeline
