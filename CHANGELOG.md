...
- **CRITICAL: rdf_edges IDENTITY Syntax**: Fixed SQL standard auto-increment syntax not supported by IRIS (MULTIPLE attempts needed)
  - Bug Evolution:
    ...
    - v1.1.6 FINAL: `BIGINT NOT NULL IDENTITY` with `PRIMARY KEY (edge_id)` constraint - **WORKS**
  ...
  - IRIS Errors:
    ...
    - "ERROR #5373" (attempt 2)
  - Fix: IRIS requires IDENTITY as column property with PRIMARY KEY as separate table constraint
  - Correct syntax: `BIGINT NOT NULL IDENTITY, ... PRIMARY KEY (col)`
  - Location: `iris_vector_graph/schema.py:124-129`, `sql/schema.sql:22-27`
