...
- **CRITICAL: rdf_edges IDENTITY Syntax**: Fixed SQL standard auto-increment syntax not supported by IRIS
  - Bug: Used `BIGINT GENERATED BY DEFAULT AS IDENTITY` (SQL standard) instead of IRIS-native `BIGINT IDENTITY`
  - Impact: **rdf_edges table creation failed**, causing kg_NEIGHBORHOOD_EXPANSION() and kg_PERSONALIZED_PAGERANK() to fail
  - IRIS Error: "Invalid SQL statement - ) expected, IDENTIFIER (GEN"
  - Fix: Changed to IRIS-native syntax `BIGINT IDENTITY PRIMARY KEY`
  - Location: `iris_vector_graph/schema.py:124`, `sql/schema.sql:22`
  - Discovered by: HippoRAG2 team during v1.1.6 testing - **v1.1.6 initial release never worked!**
