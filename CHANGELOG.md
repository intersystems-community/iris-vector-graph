...
- **CRITICAL: rdf_edges IDENTITY Syntax**: Fixed SQL standard auto-increment syntax not supported by IRIS (FOUR attempts needed)
  - Bug Evolution:
    - v1.1.6 initial: `BIGINT GENERATED BY DEFAULT AS IDENTITY` (SQL standard) - FAILED
    - v1.1.6 attempt 2: `BIGINT IDENTITY PRIMARY KEY` (incomplete IRIS syntax) - FAILED
    - v1.1.6 attempt 3: `BIGINT NOT NULL IDENTITY` + `PRIMARY KEY (col)` with `IF NOT EXISTS` - FAILED (ERROR #5373)
    - v1.1.6 FINAL: Same syntax BUT removed `IF NOT EXISTS` from table creation - **SHOULD WORK**
  ...
  - IRIS Errors:
    ...
    - "ERROR #5373" (attempts 2 & 3 - same error!)
  - Fix: IRIS doesn't support `IF NOT EXISTS` with IDENTITY columns - must use plain `CREATE TABLE`
  - Correct syntax: `CREATE TABLE rdf_edges( edge_id BIGINT NOT NULL IDENTITY, ... PRIMARY KEY (edge_id) )`
  - Key learning: IDENTITY requires separate PRIMARY KEY constraint AND no IF NOT EXISTS
  - Location: `iris_vector_graph/schema.py:124-131`, `sql/schema.sql:22-28`
  - Reference: `sql/fraud/schema.sql` (working example without IF NOT EXISTS)
  - Discovered by: HippoRAG2 team during v1.1.6 testing - **v1.1.6 initial releases never worked!**
