...
- [ ] T012 [US1] In `iris_vector_graph/cypher/translator.py`, extend the `var_length_paths` dict appended in `translate_match_clause` (line ~1102): add `"shortest": rel.variable_length.shortest` and `"all_shortest": rel.variable_length.all_shortest` and `"direction": "both" if rel.direction == ast.Direction.BOTH else "outgoing"` to the dict; also store `"src_var"` and `"dst_var"` as the variable names of the source and target nodes in the pattern
- [ ] T013 [US1] Add `Graph.KG.Traversal.ShortestPathJson(srcId, dstId, maxHops, predsJson, findAll)` ClassMethod to `iris_src/src/Graph/KG/Traversal.cls`: (1) if `srcId = dstId` return `[{"nodes":[$srcId], "rels":[], "length":0}]`; (2) BFS using `^||SP.parent(nodeId) = $ListBuild(parentId, relType)` for parent pointers, `^||SP.frontier` and `^||SP.seen` process-private globals; (3) expand frontier by iterating `^KG("out", s, p, o)` (and `^KG("in", s, p, o)` when `findAll` direction is both); filter by `predsJson` if non-empty; (4) when `dstId` first reached: backtrack `^||SP.parent` chain to reconstruct ordered nodes + rels arrays; (5) if `findAll=0` return immediately; if `findAll=1` continue BFS at same depth collecting all minimum-length paths; (6) return JSON string `[{"nodes":[...],"rels":[...],"length":N},...]` or `[]` if no path found; Kill all `^||SP.*` globals before returning; compile into `iris_vector_graph` container
- [ ] T014 [US1] Add `_execute_shortest_path_cypher(self, sql_query, parameters)` method to `IRISGraphEngine` in `iris_vector_graph/engine.py`: (1) extract `vl = sql_query.var_length_paths[0]`; (2) extract `source_id` and `target_id` by looking up `vl["src_var"]` and `vl["dst_var"]` in the SQL parameters list or `parameters` dict; (3) call `_call_classmethod(self.conn, "Graph.KG.Traversal", "ShortestPathJson", source_id, target_id, vl["max_hops"], preds_json, 1 if vl["all_shortest"] else 0)`; (4) parse JSON response; (5) return `{"columns": ["p"], "rows": [[row["nodes"], row["rels"], row["length"]]] for row in results}` (or `{"columns":["p"],"rows":[]}` if empty)
...
- [ ] T022 [US2] In `iris_vector_graph/engine.py`, extend `_execute_shortest_path_cypher` to detect which columns are requested: parse the `sql_query` metadata to check if RETURN clause contains `nodes(p)`, `relationships(p)`, or `length(p)` calls; build result rows accordingly — `nodes(p)` → the nodes array from BFS result, `relationships(p)` → rels array, `length(p)` → length integer, `p` → full JSON object `{"nodes":[...],"rels":[...],"length":N}`
