...
- **FR-006**: With `force=False`, `embed_edges` MUST load all existing `(s, p, o_id)` tuples from `kg_EdgeEmbeddings` into a Python `set` and skip any edge whose tuple is in that set; with `force=True` MUST re-embed all matched edges (DELETE + INSERT). This mirrors the `embed_nodes` pattern and is correct for graphs up to ~1M edges (~280MB peak memory for the skip-set at that scale; 300k edges ≈ 85MB, well within bounds for a standard server). Callers with >1M embeddings should use `where` to embed in predicate-scoped batches.
