...
- [ ] T001 Verify `iris-enterprise-2026` container accessible and has LDBC SF10 data — use `IRISContainer.attach("iris-enterprise-2026")` to resolve port dynamically; skip benchmark tests if unavailable
- [ ] T002 Verify `arno-builder` Docker image exists (`docker images | grep arno-builder`) and `libarno_callout.so` deployed to `/tmp/` on enterprise container
...
- [ ] T008 [US2] Write `test_rebuild_nkg_under_30s` in `tests/e2e/test_ic3_exact_count.py` — `engine.rebuild_nkg()` completes in ≤30s on enterprise IRIS; use `IRISContainer.attach("iris-enterprise-2026")` to connect; skip if container unavailable or has no LDBC data
- [ ] T009 Confirm T004-T008 all FAIL — T004/T005/T006/T007 fail because `KHop2CountExact` method does not exist (expect `<METHOD DOES NOT EXIST>` error or import error); T008 may error or skip
...
- [ ] T012 Cross-build `libarno_callout.so` using `arno-builder` Docker image per spec 094 procedure — `docker run --rm -v "<workspace_root>/..:/src" -w /src/<arno-callout-relative-path> arno-builder aarch64`; output at `~/ws/arno/target/aarch64-unknown-linux-gnu/release/libarno_callout.so`
...
- [ ] T014 Deploy new `.so` to enterprise container: `docker cp <path-to-so> iris-enterprise-2026:/tmp/libarno_callout.so` — path verified from T012 output
