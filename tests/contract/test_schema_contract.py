...
class TestSchemaContract:
    ...
    def test_identity_syntax_is_iris_native(self):
        """
        Contract: AUTO-INCREMENT columns MUST use IRIS-native IDENTITY syntax

        IRIS syntax: BIGINT NOT NULL IDENTITY, ... PRIMARY KEY (col)
        CRITICAL: IRIS doesn't support IF NOT EXISTS with IDENTITY columns
        ...
        This test prevents regressions where SQL standard syntax
        is used instead of IRIS-native syntax.
        """
        schema_sql = GraphSchema.get_base_schema_sql()

        # Check for SQL standard syntax (NOT supported by IRIS)
        invalid_patterns = [
            ...
            r'AUTO_INCREMENT',
            r'AUTOINCREMENT',
            r'IDENTITY\s+PRIMARY\s+KEY'  # This is also not supported by IRIS
        ]

        for pattern in invalid_patterns:
            matches = re.findall(pattern, schema_sql, re.IGNORECASE)
            assert len(matches) == 0, (
                f"Found SQL standard auto-increment syntax that IRIS doesn't support: {pattern}. "
                f"Use IRIS-native syntax: BIGINT NOT NULL IDENTITY with PRIMARY KEY (col) constraint"
            )

        # Verify IRIS-native IDENTITY syntax is used
        # Pattern: BIGINT NOT NULL IDENTITY (without PRIMARY KEY inline)
        iris_identity_pattern = r'BIGINT\s+NOT\s+NULL\s+IDENTITY'
        matches = re.findall(iris_identity_pattern, schema_sql, re.IGNORECASE)

        assert len(matches) >= 1, (
            "Schema should use IRIS-native IDENTITY syntax for auto-increment columns. "
            "Expected: BIGINT NOT NULL IDENTITY with separate PRIMARY KEY (col) constraint"
        )

        # CRITICAL: Verify IDENTITY tables don't use IF NOT EXISTS
        # IRIS doesn't support IF NOT EXISTS with IDENTITY columns
        identity_with_if_not_exists = re.search(
            r'CREATE\s+TABLE\s+IF\s+NOT\s+EXISTS\s+\w+\s*\([^)]*IDENTITY[^)]*\)',
            schema_sql,
            re.IGNORECASE | re.DOTALL
        )

        assert identity_with_if_not_exists is None, (
            "Found CREATE TABLE IF NOT EXISTS with IDENTITY column. "
            "IRIS doesn't support IF NOT EXISTS with IDENTITY columns. "
            "Use CREATE TABLE (without IF NOT EXISTS) and handle 'already exists' errors in execute_schema_sql()."
        )
